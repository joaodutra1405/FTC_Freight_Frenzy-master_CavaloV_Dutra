...
public class T265LocalizerRR implements Localizer {
    ...
    public Pose2d getPoseEstimate() {
        ...
        if (up != null) {
            ...
            mPoseEstimate =  new Pose2d(rawPose.getX() + poseOffset.getX(), rawPose.getY() + poseOffset.getY(), rawPose.getHeading() + poseOffset.getHeading()); //offsets the pose to be what the pose estimate is;
        } else {
...
//        RobotLog.v("POSE ESTIMATE " + mPoseEstimate.toString());
    ...
    public void setPoseEstimate(@NotNull Pose2d pose2d) {
        ...
        //Pose2d pose2d_nz = new Pose2d(pose2d.getX(),pose2d.getY(),0);
        RobotLog.v("SETTING POSE ESTIMATE TO " + pose2d.toString());
        poseOffset = pose2d.minus(rawPose);
        //poseOffset = new Pose2d(poseOffset.getX(), poseOffset.getY(), 0);//pose2d.getHeading() - rawPose.getHeading()
